use crate::manage_file::{create_file, read_file_all_as_bytes, read_every_line};
use crate::metadatas::{latest_commit_hash};
use super::util;

use std::path::{Path, PathBuf};

pub fn create_blob(path: &Path) -> std::io::Result<String> {
    if let Some(latest_hash) = latest_commit_hash()? {
        ...
        if let Some(record) = table.search_record(&latest_hash, path) {
            ...
            let latest_file = read_file_all_as_bytes(path)?;
...
fn generate_blob(path: &Path) -> std::io::Result<String> {
    let src = read_file_all_as_bytes(path)?;
    let hash = util::generate_hash(&src);
    // write object
...
pub fn get_object_data(hash: &str) -> std::io::Result<Vec<u8>> {
    ...
    match read_file_all_as_bytes(path) {
        Ok(buffer) => util::decode_by_zlib(&buffer),
        Err(e) => panic!("hash not found: {}", e)
    }
}

pub fn generate_tree(path: &Path, hashes: Vec<(Entry, String, PathBuf)>) -> std::io::Result<String> {
    let mut lines = Vec::new();
    let head = format!("{}", path.to_str().unwrap());
    lines.push(head);
    for (e, hash, path) in hashes {
        println!("DEBUG @generate_tree: path=>{:?}, hash=>{}", path, hash);  // DEBUG: To check tree's row.
        let file_type = match e {
    ...
    let hash = util::generate_hash(text.as_bytes());
